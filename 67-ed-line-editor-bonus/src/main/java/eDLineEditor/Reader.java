...
public class Reader {
     ...
     *
     * @param str    需要搜索的字符串
     * @param op 正向（/str/）为1，反向（?str?）为0
     * @return 所在行数，如果不存在就返回-1000000使其报错
     */
    private int FileFinder(String str, int op) {
        List<String> memoryList = SaveArea.memory;
        int line = SaveArea.currentLine;
        if (line < SaveArea.memory.size() + 2 && line >= 0) {
            if (op == 1) {
    ...
    private int SingleAddressConverter(String access) {
        ...
        if (Pattern.matches("\\+[0-9]*", access)) {
            return SaveArea.currentLine + Integer.parseInt(access.substring(1));
        }
        if (Pattern.matches("-[0-9]*", access)) {
            return SaveArea.currentLine - Integer.parseInt(access.substring(1));
        }
        ...
        if (access.equals(";")) {
            return Integer.parseInt(MultiAddressConverter(Integer.toString(SaveArea.currentLine) + ",$"));
        }//获得当前行到末尾

        access = access.replaceAll("\\.", Integer.toString(SaveArea.currentLine));
        access = access.replaceAll("\\$", Integer.toString(getFileLines()));
        char[] access1 = access.toCharArray();
        int num1 = SaveArea.currentLine;
        ScriptEngine jse = new ScriptEngineManager().getEngineByName("JavaScript");
        try {
            return (int) jse.eval(access);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    private String Replacer(String access) {
        ...
        if (access.charAt(0) == ',') {
            access = "1,$" + access.substring(1);
        }
        String a = Integer.toString(SaveArea.currentLine) + ",$";
     ...
     */
    public Command setCommand(String string) {
        if (string.equals("u")) {
            return new CommandU();
        }//撤销操作
        ...
        int num1 = SaveArea.currentLine, num2 = SaveArea.currentLine;
        int n = getFileLines() - SaveArea.currentLine;
        if (commandType.equals("k")) {
            ...
            int inede = 1;
            for (int i = 0; i < SaveArea.memory.size(); i++) {
                if (SaveArea.memory.get(i).contains("‘" + tempList[0])) {
                    inede = 0;
                    break;
                }
            }
        ...
        switch (commandType) {
            case "a":
                return new CommandA(num1);
            case "i":
                return new CommandI(num1);
        }
        ...
        switch (commandType) {
            case "c":
                return new CommandC(num1, num2);
            case "d":
                return new CommandD(num1, num2);
            case "p":
                return new CommandP(num1, num2);
        }
        ...
        if (commandType.equals("w") || commandType.equals("W")) {
            String file = SaveArea.defaultFilename;
            num1 = 1;
            num2 = getFileLines();
            if (string.length() == 1) {
            } else {
        ...
        if (commandType.equals("z")) {
            num1 = SaveArea.currentLine + 1;
            // System.out.println(string);
            if (string.equals("z")) {
                return new CommandZ(SaveArea.currentLine + 1, getFileLines() - SaveArea.currentLine - 1);
            } else {
        ...
        if (commandType.equals("m") || commandType.equals("t")) {
            num1 = SaveArea.currentLine;
            num2 = SaveArea.currentLine;
            int num3 = SaveArea.currentLine;
            if (List.length > 1) {
            ...
            switch (commandType) {
                case "m":
                    return new CommandM(num1, num2, num3);
                case "t":
                    return new CommandT(num1, num2, num3);
            }
        }
            ...
            if (string.equals("s")) {
                ...
                if (string.startsWith("s")) {
                    if (string.endsWith("/")) {
                        ...
                        reflict = (temp).substring(1);
                    } else {
                        reflict = string.substring(1);
                    }
    ...
    public State check(String command) {
        this.str = command;
        this.state = SaveArea.state;
        Scanner sc = SaveArea.sc;
        int is = 1;
        if (SaveArea.memory.size() != SaveArea.saveList.get(0).size()) {
            is = 0;
        }
        else {
            for (int i = 0; i < SaveArea.memory.size(); i++) {
                if (!SaveArea.memory.get(i).equals(SaveArea.saveList.get(0).get(i))) {
                    is = 0;
                    break;
                }
            }
        }
        if (str.equals("q") && SaveArea.state != State.Save && is == 0) {
            System.out.println("?");
            SaveArea.state = State.Check;
